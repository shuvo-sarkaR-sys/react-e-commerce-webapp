...
const Navbar = () => {
  ...
  return (
    <div className='bg-[#081621] sticky z-10 top-0 w-[100%] h-20 flex justify-between px-3 md:px-8 sm:p-1  items-center  text-white'>
       <Link to='/' className='flex items-center gap-3'><a href='#' >
        <p className='text-xl md:text-4xl font-mono font-semibold'>SARKAR </p></a>
        <img className='md:w-16 w-7' src={logo} alt="home" /> </Link>
      <div>
        <input onChange={handleInput} className='sm:h-10 h-8 text-black w-[100px]  rounded-l-md   md:w-[250px] lg:w-[500px]  ' type="text" />
      <Link to='search'><button onClick={handleSearch} className='sm:h-10 h-8 w-12  md:w-16 bg-orange-400 rounded-r-md'>Search</button> </Link>
 </div>
